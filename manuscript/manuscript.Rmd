...
to provide an example that can be reproduced
using an automatically generated synthetic dataset. 
...
apa_table(res$fit[, c("Fit", "Com", "BF.u", "BF.c", "PMPa", "PMPb")], format.args = list(na_string = ""))
```
...
relative to the unconstrained alternative hypothesis, which places no (in)equality constraints on the parameters.
Table \@ref(tab:res) indicates that all of these Bayes factors, BF$_{.u}$ are quite small,
with the largest being $`r report(res$fit$BF.u[4], equals = F)`$ - thus indicating that none of the hypotheses is strongly preferred over the unconstrained hypothesis.
Second, we evaluate Bayes factors for each informative hypothesis $H_i$
relative to the complement of that hypothesis, which states that any other ordering of parameters is true, $H_c: !H_i$.
For $H_1$, the Bayes factor was BF$_1 `r report(res$fit$BF.c[1])`$, indicating no evidence for or against the hypothesis that parents' support positively predicts emotion regulation.
...
Finally, overall, there was limited evidence in favor of the combination of these three hypotheses, relative to their complement, BF$_4 `r report(res$fit$BF.c[4])`$.
...
and for an unconstrained hypothesis, $H_u$, which will be preferred if none of the informative hypotheses are supported by the data.
It should be noted that in this particular example,
comparing posterior model probabilities is not very meaningful,
because the first three hypotheses are constitu
